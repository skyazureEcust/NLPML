...
def confusionFunc(y_test, pred, labels, method):
    ...
    plot_confusion_matrix(cm, labels,method, title='SVM_confusion matrix')
...
def machineLearningWays(labels, method):
    ...
    tunned_parameters = [{'kernel': ['rbf'], 'gamma': [1, 0.1, 0.01], 'C': [1, 5, 10]},
    ...
    for score in scores:
        ...
        clf = GridSearchCV(SVC(), tunned_parameters, cv=3, scoring=score)
        # clf = GridSearchCV(SVC(), tunned_parameters, cv=3, scoring='%s_macro' % score)
        clf.fit(x_train, y_train)
        # clf.fit(x, y)
