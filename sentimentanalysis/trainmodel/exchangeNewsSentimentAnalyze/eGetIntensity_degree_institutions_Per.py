                ...
                if sum == 10:
                    ...
                    for each in fsrc.readlines():
                        ...
                        if flag == 1:
                            ...
                            #         data2) + '\n')
                    k2 = std2 / row
                    # print('实验2权值设置HD=%d,MD=%d,LD=%d的V值=%f：'%(HD,MD,LD,k2))
                    if k2 > max:
                        max = k2
                        HD_max = HD
                        MD_max = MD
                    ...
                    if k2 < min:
                        ...
                        MD_min = MD
                        LD_min = LD
    print('总结-----实验2权重设置最大V=%f,权值为HD=%d,MD=%d,LD=%d'%(max,HD_max,MD_max,LD_max))
    print('总结-----实验2权重设置最小V=%f,权值为HD=%d,MD=%d,LD=%d'%(min,HD_min,MD_min,LD_min))
            ...
            for k in range(1, 10):
                ...
                if sum == 10:
                    ...
                    if k3 < min:
                        ...
                        MEW_min = MEW
                        LEW_min = LEW
    print('总结-----实验3权重设置最大V=%f,权值为HEW=%d,MEW=%d,LEW=%d' % (max, HEW_max, MEW_max, LEW_max))
    print('总结-----实验3权重设置最小V=%f,权值为HEW=%d,MEW=%d,LEW=%d' % (min, HEW_min, MEW_min, LEW_min))
                        ...
                        for p in range(1,10):
                            ...
                            if sum == 10:
                                ...
                                if k4 < min:
                                    ...
                                    MD_min = MD
                                    LD_min = LD
    print('总结-----实验4权重设置最大V=%f,权值为HEW=%d,MEW=%d,LEW=%d,HD=%d,MD=%d,LD=%d' % (max, HEW_max, MEW_max, LEW_max,HD_max, MD_max, LD_max))
    print('总结-----实验4权重设置最小V=%f,权值为HEW=%d,MEW=%d,LEW=%d,HD=%d,MD=%d,LD=%d' % (min, HEW_min, MEW_min, LEW_min,HD_min, MD_min, LD_min))
...
if __name__ == "__main__":
    ...
    #说明
    # 设置1 - 10的权重范围（考虑所有数据 - --没有百分数时设置0.001（大多数百分数处于千分位上）
    # 设置1-10的权重范围，步长1，（考虑只有程度词和政策类词汇）-----没有百分数时是指为0.0001（1个基点）
