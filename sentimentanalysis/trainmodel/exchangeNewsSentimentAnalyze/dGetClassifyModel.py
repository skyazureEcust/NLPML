...
def machineLearningWays(method):
    ...
    for j in range(6):
        ...
        if method == 'NaiveBayes':
            ...
            acc, pre, rec, f1 = evaluate(y_test, LR(x_train, y_train, x_test))
...
def figureImage(method, xList,accuracyList,precisionList, recallList, fList):
    ...
    plt.plot(xList, accuracyList, label='accuracy')
    plt.plot(xList, precisionList, label='precision')
    plt.plot(xList, recallList, label='recall')
    plt.plot(xList, fList, label='F1')

    plt.xlabel('percentage of testing set')
    plt.ylabel('value')
    plt.title(method)
    plt.legend(loc='upper left')
    # plt.savefig('../eImage/%s.png'%method)
    plt.savefig('../eImage2/%s.png'%method)

if __name__ == "__main__":
    # #读取词向量后的文件
    # # fname = r'../eFiles/cLabelToVector_word2vec'
...
#读取词向量后的文件
    # fname = r'../eFiles/cLabelToVector_word2vec'
    fname = r'../eFiles2/cLabelToVector_word2vec_Only_Word.txt'
    # fname = r'../eFiles2/cLabelToVector_word2vec_use_sentiWord.txt'
    # fname = r'../eFiles2/cLabelToVector_word2vec_use_sentiWordAndDegWord.txt'
    fsrc = open(fname,'r',encoding='utf-8')
    x,y = generateDataSet(fsrc)
    #训练集的特征列，测试集的特征列，训练集的label列，测试集的label列
    y = column_or_1d(y, warn=True)
    ...
    #确定参数 test_size,random_state
    methodList = ['NaiveBayes', 'SVM', 'RandomForest','LogisticRegression']
    for method in methodList:
        # xList, accuracyList, precisionList, recallList, fList, auc_rocList = machineLearningWays(method)
        xList, accuracyList, precisionList, recallList, fList = machineLearningWays(method)
        figureImage(method, xList,accuracyList,precisionList, recallList, fList)
