...
# def xgBoostTrain(X_train, y_train, X_test):
...
def machineLearningWays(method):
    ...
    # 设置x轴的数据
    xList = []
    for rValue in range(0, 31, 5):
        xList.append(rValue)
        x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=rValue)
        print(method, ':')
...
# def figureImage(method, xList,accuracyList,precisionList, recallList, fList):
    ...
    plt.savefig('../eImage/dGetClassifyModel_tiaoCan_Random_state_RF/%s.png'%method)
