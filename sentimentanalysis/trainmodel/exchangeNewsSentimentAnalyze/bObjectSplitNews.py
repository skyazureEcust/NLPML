...
def main(threeObject_A, fsrc, fcomp, fsave, special_word):
    #读取文件
    objectNumber = 0
    for eachNews in fsrc.readlines():
        ...
        if len(newsContent_List2) == len(objectDict):
            ...
            for childNews, ob in zip(newsContent_List2, objectDict):
                fcomp.write(str(newsTime) + '\t' + childNews + '\t' + ob[0] + '\n')
                fsave.write(str(newsTime) + '\t' + childNews + '\t' + ob[0] + '\n')
        else:
            # print('len(newsContent_List2)',len(newsContent_List2),newsContent_List2)
            # print('len(objectDict)',len(objectDict),objectDict)
            fcomp.write(str(newsTime) + '\t' + newsContent +  '\n')
...
if __name__ == "__main__":
    ...
    #保存对比文件,新闻中有评价对象则在后面写入，没有就原样写入
    file = '../eFiles/bObjectSplitNews_compFile.txt'
    fcomp = open(file,'w',encoding='utf-8')

    # 保存只有评价对象的句子
    file = '../eFiles/bObjectSplitNews_objectFile.txt'
    fsave = open(file, 'w', encoding='utf-8')

    main(threeObject_A,fsrc,fcomp, fsave, special_word)
