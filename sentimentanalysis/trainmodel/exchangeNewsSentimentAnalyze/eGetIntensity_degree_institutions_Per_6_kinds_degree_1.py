...
def compare(fname,f2,f2save):


    for eachf,each2 in zip(fname.readline(),f2.readlines()):
        eachfv = eachf.strip().split('\t')
        each2v = each2.strip().split('\t')


def generateDataSet(fsrc,fsave):
    #计算各个极性的数量
    zero_number = 0
    positive_number = 0
    negtive_number = 0
    ...
    for each in fsrc.readlines():
        ...
        if each == 1:
            ...
            if positive_number <= 1160:
                fsave.write(str(eachitems[0]) + '\t' + eachitems[1] + '\t' + eachitems[2] + '\t' + eachitems[3] + '\t' + eachitems[4] + '\n')

        if each == -1:
            negtive_number = negtive_number + 1
            if negtive_number <= 1160:
                fsave.write(str(eachitems[0]) + '\t' + eachitems[1] + '\t' + eachitems[2] + '\t' + eachitems[3] + '\t' + eachitems[4]+ '\n')
...
if __name__ == "__main__":
    ...
    file1 =  '../eFiles2/jisuanji/eGetIntensity_degree.txt' #计算机工程与设计
    ...
    file2 = '../eFiles2/jisuanji/eGetIntensity_degree_institutions.txt'  #计算机工程与设计
    ...
    file3 = '../eFiles2/jisuanji/eGetIntensity_degree_institutions_Per.txt'  #计算机工程与设计
