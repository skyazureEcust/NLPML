...
def data_distribute(fobj,fsave, highWeightList, middleWeightList, lowWeightList, degreeHighList, degreeLowList):
    ...
    for every in fobj.readlines():
        ...
        middle = float(data) * int(pol)
        intensity = '%0.4f'%(middle)
        fsave.write(str(newsTime) + '\t' + newsContent + '\t' + obj + '\t' + pol + '\t' + str(intensity) + '\n')
    fsave.close()
...
if __name__ == "__main__":
    ...
    # #people1
    # file = '../eFiles/eaSetIntensityValue_percentage_first_people_label.txt'
    ...
    # people2
    file = '../eFiles/eaSetIntensityValue_percentage_second_people_label.txt'
    fsave = open(file, 'w', encoding='utf-8')
    # 第二种---people2
    firstWeight = 2
    secondWeight = 1.5
