...
def func_degree_constant(fsrc,fsave,sentiWord,deg1, deg2, deg3,clf, word_vec):
    ...
    for each in fsrc.readlines():
        ...
        middleValue = MP * deg * int(pol)
        # data = getSentimentIntensity(middleValue, pol)
        data = middleValue
        std = std + abs((float(quaRaw)) - (float(data)))
        fsave.write(str(newsTime) + '\t' + newsContent + '\t' + obj + '\t' + pol + '\t' + quaRaw +  '\t' + str(data) + '\n')
    print('行数1',row)
    if row != 0:
        k = std / row
    else:
                ...
                if (sum == scope) & (i > j+2 > k+2 > 10):
                    ...
                    for each in fsrc.readlines():
                        ...
                        institutions = sentimentIntensity_institutions(newsContent, highWeightList,
                                                                       ...
                                                                       LEW)
                        middleValue = MP * deg * institutions * int(pol)
                        data = middleValue
...
def func_degree_institutions_write_txt(fsrc, fsave,sentiWord, deg1, deg2, deg3, clf, word_vec, HD_min, MD_min, LD_min,HEW_min, MEW_min, LEW_min):
    global scope, scopeHalf
    row = 0
    std = 0
    fsrc.seek(0)
    for each in fsrc.readlines():
        ...
        data = middleValue
        fsave.write(
            str(newsTime) + '\t' + newsContent + '\t' + obj + '\t' + pol + '\t' + quaRaw + '\t' + str(data) + '\n')

    print('行数3',row)
    fsave.close()
    # fsrc.close()

def func_degree_institutions_Per(fsrc,fsave,sentiWord,highWeightList, middleWeightList, lowWeightList,clf, word_vec, HD_min, MD_min, LD_min, HEW_min, MEW_min, LEW_min,attrList):
    row = 0
    std = 0
    fsrc.seek(0)
    for each in fsrc.readlines():
        ...
        middleValue = MP * deg * institutions * Per * int(pol)
        data = middleValue
        std = std + abs((float(quaRaw)) - (float(data)))
        fsave.write(
            str(newsTime) + '\t' + newsContent + '\t' + obj  + '\t' + attr + '\t' + pol + '\t' + quaRaw + '\t' + str(data) + '\n')
...
if __name__ == "__main__":
    ...
    file1 =  '../eFiles/eGetIntensity_degree.txt'
    fsave1 = open(file1, 'w', encoding='utf-8')
    HD_min, MD_min, LD_min = func_degree_constant(fsrc,fsave1,sentiWord,deg1, deg2, deg3,clf, word_vec)


    HEW_min, MEW_min, LEW_min = func_degree_institutions(fsrc,sentiWord,deg1, deg2, deg3,clf, word_vec,HD_min, MD_min, LD_min)


    file2 = '../eFiles/eGetIntensity_degree_institutions.txt'
    fsave2 = open(file2, 'w', encoding='utf-8')
    func_degree_institutions_write_txt(fsrc, fsave2,sentiWord, deg1, deg2, deg3, clf, word_vec, HD_min,
                                                         MD_min, LD_min,HEW_min, MEW_min, LEW_min)


    file3 = '../eFiles/eGetIntensity_degree_institutions_Per.txt'
    fsave3 = open(file3, 'w', encoding='utf-8')
    func_degree_institutions_Per(fsrc,fsave3,sentiWord,highWeightList, middleWeightList, lowWeightList,clf, word_vec, HD_min, MD_min, LD_min, HEW_min, MEW_min, LEW_min, attrList)
