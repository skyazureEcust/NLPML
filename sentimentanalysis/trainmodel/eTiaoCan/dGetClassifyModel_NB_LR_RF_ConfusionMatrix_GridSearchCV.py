...
def plot_confusion_matrix(cm, labels,method, title='Confusion Matrix', cmap=plt.cm.binary):
# def plot_confusion_matrix(cm, labels,method, title='Confusion Matrix', cmap=plt.cm.red):
    ...
    # plt.show()
    plt.savefig('../eImage/d_RF_GridSearchCV_confusion_matrix_noBar_Red_%s.png'%method)
...
def confusionFunc(y_test, pred, labels, method):
    ...
    plot_confusion_matrix(cm, labels,method, title='%s_confusion matrix'%method)
...
def machineLearningWays(labels, method):
    ...
    #朴素贝叶斯
    clf = NaiveBayesTrain(x_train, y_train, x_test)
    clf.fit(x_train, y_train)
    # clf.fit(x, y)
    ...
    #生成训练模型
    joblib.dump(clf, '../eSentimentModel/LogisticReg_W_model.m')
    # 生成混淆矩阵
    confusionFunc(y_true, y_pred, labels, 'LogisticRegression')
    ...
    print(x_test)
    proValue = clf.predict_proba(x_test)
    print('proValue', proValue)
